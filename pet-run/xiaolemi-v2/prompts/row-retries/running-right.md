Create Codex pet row `running-right` for `xiaolemi`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: 保持现有角色身份：粉色短发与长刘海，白色天使翅膀，白紫色服装，深蓝粗描边，粉色脸颊与眼睛，星形发饰；小巧圆润的Q版比例；画板和画笔是核心道具；保留原有日系可爱画风，不添加文字、场景或新角色。. Preserve silhouette, face, palette, material, proportions, markings, and props.
